  ...
  )
{
  ...
  return SmmCommunicationCommunicate (
           (EFI_MM_COMMUNICATION_PROTOCOL *)&mMmSupvCommunication,
           CommBuffer,
           CommSize
           );
}
  ...
  )
{
  ...
  if ((UINTN)This == (UINTN)&mMmSupvCommunication) {
    ...
    if (EFI_SIZE_TO_PAGES (TempCommSize) > mMmSupvCommonBufferPages) {
      if (CommSize != NULL) {
        *CommSize = EFI_PAGES_TO_SIZE (mMmSupvCommonBufferPages);
      }
    ...
    if (EFI_SIZE_TO_PAGES (TempCommSize) > mMmUserCommonBufferPages) {
      if (CommSize != NULL) {
        *CommSize = EFI_PAGES_TO_SIZE (mMmUserCommonBufferPages);
      }
  ...
  //
  if (CommSize != NULL) {
    *CommSize = gMmCorePrivate->BufferSize;
  }

  if (CommunicateHeader != CommBuffer) {
    CopyMem (CommBuffer, CommunicateHeader, gMmCorePrivate->BufferSize);
  }

  return gMmCorePrivate->ReturnStatus;
}
  ...
  )
{
  ...
  mCommunicateHeader = (EFI_SMM_COMMUNICATE_HEADER *)mMmUserCommonBuffer;
...
/**
  Helper function to notify MM core to dispatch MM drivers.
  ...
  )
{
  ...
  while (TRUE) {
    ...
    //
    CopyGuid (&(mCommunicateHeader->HeaderGuid), (EFI_GUID *)&gMmSupervisorDriverDispatchGuid);
    mCommunicateHeader->MessageLength = 1;
    mCommunicateHeader->Data[0]       = 0;
    ...
    //
    Size   = sizeof (EFI_SMM_COMMUNICATE_HEADER);
    Status = SmmCommunicationCommunicate ((EFI_MM_COMMUNICATION_PROTOCOL *)&mMmSupvCommunication, mCommunicateHeader, &Size);
  ...
  )
{
  ...
  //
  Size = sizeof (EFI_SMM_COMMUNICATE_HEADER);
  SmmCommunicationCommunicate ((EFI_MM_COMMUNICATION_PROTOCOL *)&mMmSupvCommunication, mCommunicateHeader, &Size);
  ...
  )
{
  ...
  Status = RequestHeader->Result;
  if (EFI_ERROR (Status)) {
    DEBUG ((DEBUG_ERROR, "%a Supervisor failed to reply useful version information - %r!!\n", __FUNCTION__, Status));
    return Status;
  }
  ...
  )
{
  ...
  if (PcdGet64 (PcdLoadModuleAtFixAddressEnable) != 0) {
    ...
    if (!EFI_ERROR (Status)) {
      ...
      PageCount = 0;
      //
      // Reserved Smram Region for SmmCore is not used, and remove it from MmramRangeCount.
      //
  ...
  )
{
  ...
  MmramRangeCount = Size / sizeof (EFI_SMRAM_DESCRIPTOR);
  ...
  //   ......
  //
  MaxCount = MmramRangeCount + 2 * SmramReservedCount;
    ...
    for (Index = 0; (Index < MmramRangeCount) && !Rescan; Index++) {
      ...
      if (SmramRanges[Index].PhysicalSize != 0) {
        ...
        if (!Rescan) {
          ...
          //
          CopyMem (&TempSmramRanges[TempMmramRangeCount++], &SmramRanges[Index], sizeof (EFI_SMRAM_DESCRIPTOR));
  ...
  // Sort the entries
  //
  FullSmramRanges = AllocateZeroPool ((TempMmramRangeCount + AdditionMmramRangeCount) * sizeof (EFI_SMRAM_DESCRIPTOR));
  ASSERT (FullSmramRanges != NULL);
  *FullMmramRangeCount = 0;
  do {
    ...
    CopyMem (&FullSmramRanges[*FullMmramRangeCount], &TempSmramRanges[Index], sizeof (EFI_SMRAM_DESCRIPTOR));
  ...
  )
{
  ...
  while (GuidHob.Guid != NULL) {
    ...
    if (CommRegionHob->MmCommonRegionType == MM_USER_BUFFER_T) {
      ...
      mMmSupvCommunication.CommunicationRegion.Type          = EfiRuntimeServicesData;
      mMmSupvCommunication.CommunicationRegion.PhysicalStart = (EFI_PHYSICAL_ADDRESS)mMmSupvCommonBuffer;
      mMmSupvCommunication.CommunicationRegion.VirtualStart  = (EFI_PHYSICAL_ADDRESS)mMmSupvCommonBuffer;
      mMmSupvCommunication.CommunicationRegion.NumberOfPages = mMmSupvCommonBufferPages;
      mMmSupvCommunication.CommunicationRegion.Attribute     = 0;
    }
  ...
  gMmCorePrivate->MmramRanges = (UINTN)GetFullSmramRanges (&gMmCorePrivate->MmramRangeCount);
  MmramRanges                 = (EFI_MMRAM_DESCRIPTOR *)(UINTN)gMmCorePrivate->MmramRanges;
  ...
  //
  Status = mSmmAccess->Lock (mSmmAccess);
  if (EFI_ERROR (Status)) {
    //
    // Print error message that the SMRAM failed to lock...
    //
    DEBUG ((DEBUG_ERROR, "SMM IPL could not lock SMRAM after executing MM Core\n"));
    ASSERT_EFI_ERROR (Status);
  }
  ...
  if ((mCurrentSmramRange == NULL) || EFI_ERROR (Status)) {
    ...
    //
    FreePool ((VOID *)gMmCorePrivate->MmramRanges);
