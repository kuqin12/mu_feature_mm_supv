...
impl<'t, 's> Types<'t, 's> {
    ...
    /// Returns a type by index. If the type is a class or union with size 0, it will check for a
    /// shadow definition with the real information, returning that instead.
    fn find(&self, index: TypeIndex) -> Result<Item<'t, TypeIndex>> {
        let data = self.finder.find(index)?;
        let item = data.parse()?;

        // Return the item as-is if it is anything other than a class or union, or if it is a
        // class/union that is not a zero-size forward reference. Unions are forward referenced
        // the same way classes are; without handling them the `fields` index of the forward
        // reference is used, which does not point at a field list, so members of the real
        // definition are unreachable.
        let type_name = match aggregate_shape(&item) {
            // A size-0 aggregate that is a complete definition (not a forward reference) is a
            // genuine zero-sized type, so it is returned as-is along with every other shape.
            Some((0, true, name)) => name,
            _ => return Ok(data),
        };

        // The type was a size-0 forward-reference class or union, so it should have a shadow
        // definition with the real information. Failing that, the real definition may be
        // genuinely zero sized, such as Rust's unit type `tuple$<>`; no non-zero-sized definition
        // can exist for those, so a complete definition is accepted instead. A forward reference
        // with no definition at all matches neither and falls through to the error below, so a
        // genuinely missing type is not masked.
        let mut sized = None;
        let mut complete = None;
        let mut iter = self.info.iter();
        while let Some(item) = iter.next()? {
            let Some((size, forward_reference, name)) = aggregate_shape(&item.parse()?) else {
                continue;
            };
            if name != type_name {
                continue;
            ...
            if !forward_reference {
                complete.get_or_insert(item);
            }
        }

        sized
            .or(complete)
            .ok_or_else(|| anyhow!("Symbol {} was found, but size was 0", type_name))
    }
}

/// Returns the `(size, forward_reference, name)` of a class or union, and `None` for anything else.
fn aggregate_shape(data: &TypeData) -> Option<(u64, bool, String)> {
    match data {
        TypeData::Class(d) => Some((
            d.size,
            d.properties.forward_reference(),
            d.name.to_string().to_string(),
        )),
        TypeData::Union(d) => Some((
            d.size,
            d.properties.forward_reference(),
            d.name.to_string().to_string(),
        )),
...
impl Symbol {
    ...
    fn find_field_at(
        ...
        let (field_list, parent_size) = match info.find(*id)?.parse()? {
    ...
    fn is_provably_zero_sized(info: &Types<'_, '_>, index: TypeIndex) -> bool {
        ...
        for _ in 0..MAX_MODIFIER_DEPTH {
            ...
            match data {
                ...
                data => {
                    return matches!(aggregate_shape(&data), Some((0, false, _)));
                }
            }
        }
        false
