...
class GenSeaArtifacts(IUefiHelperPlugin):
    ...
    def generate_sea_includes(
        ...
        try:
            ...
            if supervisor_name is None:
                supervisor_name = "MmSupervisorCore"
    ...
    @staticmethod
    def generate_test_aux_binary(output_path: Path, mm_supervisor_build_dir: Path, pecoff_validation_lib_build_dir: Path, workspace = None, supervisor_name=None):
...
def generate_aux_file(aux_config_path: Path, mm_supervisor_build_dir: Path, scopes: list[str], output_dir: Path, workspace = None, supervisor_name=None):
